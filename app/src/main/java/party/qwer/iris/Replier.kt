...
class Replier {
    companion object {
        ...
        fun sendMedia(room: Long, filePaths: List<String>) {
            coroutineScope.launch {
                messageChannel.send(SendMessageRequest {
                    sendMediaInternal(room, filePaths)
                })
            }
        }

        private fun sendMediaInternal(room: Long, filePaths: List<String>) {
            if (filePaths.isEmpty()) {
                System.err.println("No file paths provided, cannot send media.")
                return
            }
            ...
            if (uris.isEmpty()) {
                System.err.println("No valid URIs created, cannot send media.")
                return
            }

            // 첫 번째 파일의 확장자로 MIME 타입 결정
            val firstFile = File(filePaths[0])
            val mimeType = getMimeType(firstFile.extension.lowercase())
